...
def create_states_diff_kappa(N:int, t_list:np.ndarray, rho_0:qutip.qobj.Qobj,
                  kappa_list:np.ndarray,nb_copies:int=1) -> list[qutip.qobj.Qobj]:
    ...
    for t in t_list:
        ...
        for id_copy in range(nb_copies):
            ...
            for k in range(N): # not useful to go beyond N, the operator is 0
                kraus = kraus_op(N, k, t, kappa_list[id_copy], nb_copies, id_copy)
...
def create_states_dephasing_diff_kappa_phi(N:int, t_list:np.ndarray, rho_0:qutip.qobj.Qobj,
                            kappa_list:float,nb_copies:int=1) -> list[qutip.qobj.Qobj]:
    ...
    for t in t_list:
        ...
        for id_copy in range(nb_copies):
            rho = qutip.tensor([qutip.Qobj(np.zeros((N,N)))]*nb_copies) # 0 matrix
            for k in range(N): # not useful to go beyond N, the operator is 0
                kraus = kraus_op_dephasing(N, k, t*kappa_list[id_copy], nb_copies, id_copy)
                rho = rho + kraus * rho_init * kraus.dag()
            rho_init = rho
        rho_list.append(rho.unit())
    return rho_list
    ...
    for (i,rho) in enumerate(rho_list):
        ...
        for (j,(bs1, bs2, bs3)) in enumerate(F_list):
            ...
            if losses or dephasing: # can have both
                ...
                if losses :
                    if (losses=="multiple") : g_loss = losses_gamma[1]
...
def plots(t_list :np.ndarray, result :np.ndarray, result_errors: np.ndarray, result_th_wlabels: list, ylabel: str,
             remove_extreme : None | tuple[int] = None,
             gamma_loss:float=None, gamma_dephasing:float=None,
            show_only_th:bool=False, legend= None, box= None) -> None:
    ...
    if not show_only_th:
        plt.errorbar(t_list, np.abs(result_corr.mean(1)), result_corr.std(1), np.zeros_like(t_list), 
                    barsabove=True, elinewidth=.5,fmt='.',label="VD under $N_c$", c=colors[0], ms= 10, capsize=4)
    if (not show_only_th) and ((gamma_loss is not None) or (gamma_dephasing is not None)):
        plt.errorbar(t_list, np.abs(result_errors_corr.mean(1)), result_errors_corr.std(1), np.zeros_like(t_list), 
                    ...
                    label="VD under $N_c$" + (" + $N_l$ " if gamma_loss is not None else "") +(" + $N_d$ " if gamma_dephasing is not None else ""),
                    c=colors[6], ms=10,capsize=4)
    for (i,(result_th, label)) in enumerate(result_th_wlabels):
        plt.scatter(t_list, np.abs(result_th), label=label, marker='+', s= 70, c=colors[i+2])
    plt.plot(t_list,np.full(t_list.shape,(result_th_wlabels[0][0][0]).real),
             linestyle="--", label='Pure state', c=colors[5])

    plt.xlabel(f"$\kappa_2 t$", fontsize= 25)
    plt.ylabel(ylabel, fontsize= 25)
    if legend is not None:
        plt.legend(loc= legend, fontsize=15)
        if box is not None:
            plt.legend(loc= 'center left', bbox_to_anchor=box,fontsize=15)
